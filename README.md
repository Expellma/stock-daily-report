...
- **本地 Markdown 模板海报**：读取目录中由 ChatGPT 预先分析财报后导出的 `.md` 文件，不再调用 GPT/API，默认按 `input/templates/财报总结统一模板.md` 输出结构化财报总结；也可指定自定义模板。
...
当 OpenAI API 暂无余额或不希望联网时，可先在 ChatGPT 中手动分析财报，把结果保存为 `.md` 文件，然后让本项目离线读取这些 Markdown 内容并按模板输出财报总结海报。`md-poster` 默认读取 `input/templates/财报总结统一模板.md` 作为输出模板，也可以通过 `--template` 指定自定义 Markdown 模板：

```bash
stock-daily-report md-poster 600519.SH \
  --name 贵州茅台 \
  --report-dir input/贵州茅台 \
  --template input/templates/财报总结统一模板.md \
  --thesis "高端白酒需求与现金流质量"
```

`md-poster` 会读取模板文件并填充公司名称、Ticker、输入文件、生成日期、离线证据摘要、风险/验证点等可自动识别字段；模板中其他占位符会以“待核验/数据缺失/待补充”显式保留为尽调清单，便于继续人工完善。兼容旧命令名 `pdf-poster`，但旧命令仍输出原费雪 15 问 Markdown 海报，不会读取模板，也不会再调用 OpenAI Responses API：
...
- `--template`：仅 `md-poster` 可用，指定 Markdown 输出模板；默认 `input/templates/财报总结统一模板.md`。
- `--output-dir`：指定输出目录；默认写入 `outputs/<date>/md_reports/`。
- `--report-dir`：必填，目录中会按文件名顺序读取 `.md` / `.txt` 文件；`.pdf` 会标记为暂不支持并继续生成。
- `--thesis`：可选投资主线，会保存在离线分析对象中，便于后续扩展或二次编辑。

生成结果：

- `outputs/<date>/md_reports/<symbol>_fisher_poster.md`：`md-poster` 基于模板生成的财报总结 Markdown，默认结构与 `input/templates/财报总结统一模板.md` 保持一致；`pdf-poster` 保持输出原费雪 15 问 Markdown 海报。
